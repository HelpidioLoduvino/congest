        ...
        <div class="col-md-8">
            ...
            <div class="mt-5">
                <div class="d-flex">
                    ...
                    <h4 class="title">Reservas de espaço</h4>
                </div>
            </div>

            <div class="highlight-card" style="overflow: auto; max-height: 400px;">
                <div class="container mt-3">
                    @if (!empty($bookings))
                        @foreach ($bookings as $booking)
                        <a class="card-link" href="#" onclick="redirectToResidentBooking('{{$booking->id}}')">
                            <div class="my-div-card mb-3">
                                <div class="d-flex">
                                    <ul class="booking-list">
                                        <li><img src="{{asset('icon/user.svg')}}" alt="" width="40"></li>
                                        <small class="d-flex" style="margin-left: 10px;">
                                            <li>Morador: <br><strong> {{$booking->name}}</strong></li>
                                            <div class="vertical-green-separator-large"></div>
                                            <li>Espaço: <br> <strong>{{$booking->place}}</strong></li>
                                            <div class="vertical-green-separator-large"></div>
                                            <li>Data: <br> <strong>{{$booking->booking_date}}</strong></li>
                                            <div class="vertical-green-separator-large"></div>
                                            @if (trim($booking->status) === trim("Aguardando"))
                                            <li>{{$booking->status}}</li>
                                            @elseif (trim($booking->status) === trim("Negado"))
                                            <li style="color: red">{{$booking->status}}</li>
                                            @else
                                            <li style="color: green">{{$booking->status}}</li>
                                            @endif
                                        </small>
                                    </ul>
                                </div>
                        ...
                        @endforeach
                    @else
                        <h5 class="d-flex justify-content-center">Sem Reservas</h5>
                    @endif
                </div>
            </div>
        </div>
